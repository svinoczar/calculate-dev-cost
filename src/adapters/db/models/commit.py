from sqlalchemy import String, Text, TIMESTAMP, Float, ForeignKey, func
...
class CommitModel(Base):
    ...
    authored_at: Mapped[str | None] = mapped_column(
        TIMESTAMP(timezone=True), nullable=True
    )
    committed_at: Mapped[str | None] = mapped_column(
        TIMESTAMP(timezone=True), nullable=True
    )

    commit_type: Mapped[str | None] = mapped_column(Text, nullable=True)
    commit_type_confidence: Mapped[float] = mapped_column(Float, nullable=True)
